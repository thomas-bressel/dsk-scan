        ...
        <svg viewBox="0 0 <?= $svgW ?> <?= $svgH ?>" xmlns="http://www.w3.org/2000/svg" class="disk-svg">
            ...
            foreach ($trackMap as $trackNum => $sectors):
                ...
                foreach ($sectors as $si => $s):
                    ...
                    // Centre du secteur pour le label
            ...
            ?>
                <path d="<?= $path ?>"
                      ...
                      stroke="#0d0d1a"
                      stroke-width="0.5"
                      opacity="<?= $s['isUsed'] ? '1' : '0.45' ?>"
                      class="disk-sector-path"
                      onclick="diskSectorClick(<?= $sectorIdx ?>)">
                    <title><?= htmlspecialchars($tooltip) ?></title>
                </path>

                <?php if ($rStep >= 10 && $spt <= 18): // label visible seulement si assez de place ?>
                <text x="<?= round($lx, 1) ?>"
                      y="<?= round($ly + 4, 1) ?>"
                      text-anchor="middle"
